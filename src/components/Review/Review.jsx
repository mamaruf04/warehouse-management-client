...
const Review = () => {
  ...
  useEffect(() => {
    fetch("https://warehouse-management-2z04.onrender.com/reviewCount")
      .then((res) => res.json())
      .then((data) => setPageCount(Math.ceil(data.count / 3)));
  }, []);

  useEffect(() => {
    fetch(`https://warehouse-management-2z04.onrender.com/reviews?page=${page}`)
      .then((res) => res.json())
  ...
  const handleComment = () => {
    if (!user) {
      ...
      fetch("https://warehouse-management-2z04.onrender.com/review", {
        method: "POST",
        headers: {
